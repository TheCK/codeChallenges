...
@Solution(
    id = 20201702,
    name = "Day 17: Conway Cubes - Part 2",
...
public class Part2 {
  ...
  public static void main(String[] args) {
    ...
    for (int i = 0; i < CYCLES; ++i) {
      int[][][][] newGrid =
          new int[grid.length][grid[0].length][grid[0][0].length][grid[0][0][0].length];
      ...
      for (int newX = 0; newX < newGrid.length; ++newX) {
        for (int newY = 0; newY < newGrid[newX].length; ++newY) {
          for (int newZ = 0; newZ < newGrid[newX][newY].length; ++newZ) {
            for (int newW = 0; newW < newGrid[newX][newY][newZ].length; ++newW) {
              int neighbours = 0;
              for (int oldX = Math.max(0, newX - 1);
                ...
                for (int oldY = Math.max(0, newY - 1);
                  ...
                  for (int oldZ = Math.max(0, newZ - 1);
                      ...
                      ++oldZ) {
                    for (int oldW = Math.max(0, newW - 1);
                        oldW < Math.min(newW + 2, newGrid[newX][newY][newZ].length);
                        ++oldW) {
                      if (oldX == newX && oldY == newY && oldZ == newZ && oldW == newW) {
                        continue;
                      }

                      neighbours += grid[oldX][oldY][oldZ][oldW];
                    }
                  }
                }
              }

              if (neighbours == 3 || grid[newX][newY][newZ][newW] == 1 && neighbours == 2) {
                newGrid[newX][newY][newZ][newW] = 1;
              }
            }
          }
        }

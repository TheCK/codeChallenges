...
@Solution(id = 238, name = "Code combinations", description = "Check whether you can make words from the given letters.", url = "https://www.codeeval.com/open_challenges/238/", category = "Moderate challenges")
public class Main
{
				...
				for (int i = 0; i < arrayLines.length - 1; ++i)
				{
					for (int j = 0; j < arrayLines[0].length() - 1; ++j)
					{
						...
						if (lettersFound.size() == 4
							&& lettersFound.contains("c") && lettersFound.contains("o")
							&& lettersFound.contains("d") && lettersFound.contains("e"))
						{
							++codesInArray;
						}
					}

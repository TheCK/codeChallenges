...
@Solution(
    id = 42,
    name = "Ugly Numbers",
    description = "Count the number of expressions that can be created from a number",
...
public class Main {
  ...
  public static void main(String[] args) throws Exception {
    ...
    try (BufferedReader buffer = new BufferedReader(new FileReader(file))) {
      ...
      while ((line = buffer.readLine()) != null) {
        ...
        Long uglyNumbers = results.stream().filter(Main::isUgly).count();
  ...
  private static void generateExpressions(String expression, String remainingDigits) {
    ...
    if (l1 > 5) System.err.println(l1 + " " + expression);
  }
  ...
  private static void evaluateExpression(String expression) {
    ...
    List<String> operations =
        Arrays.stream(expression.split("[\\d]+")).collect(Collectors.toList());

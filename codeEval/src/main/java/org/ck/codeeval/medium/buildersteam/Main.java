...
@Solution(
    id = 218,
    name = "Builders team.",
    description = "Count all squares on the map.",
    url = "https://www.codeeval.com/open_challenges/218/",
...
public class Main {
  ...
  private static int count(int i, int j, int stepsRight, int stepsDown, Direction lastDirection) {
    ...
    if (grid[cell].contains("d")
        && (lastDirection == Direction.RIGHT || lastDirection == Direction.DOWN)) {
      count += count(i + 1, j, stepsRight, stepsDown + 1, Direction.DOWN);
    }
    if (grid[cell].contains("l")
        && ((lastDirection == Direction.DOWN && stepsDown == stepsRight)
            || lastDirection == Direction.LEFT)) {
      count += count(i, j - 1, stepsRight - 1, stepsDown, Direction.LEFT);
    }
    if (grid[cell].contains("u")
        && (lastDirection == Direction.LEFT || lastDirection == Direction.UP)
        && stepsRight == 0) {

...
@Solution(
    id = 215,
    name = "Double trouble",
    description = "Calculate the number of correct variants for messages.",
    url = "https://www.codeeval.com/open_challenges/215/",
      ...
      while ((line = buffer.readLine()) != null) {
        ...
        for (int i = 0; i < firstHalf.length(); ++i) {
          ...
          if ('*' == charInFirstHalf && '*' == charInSecondHalf) {
            combinations *= 2;
          } else if ('*' != charInFirstHalf
              && '*' != charInSecondHalf
              && charInFirstHalf != charInSecondHalf) {
            combinations = 0;
            break;
          }
        }

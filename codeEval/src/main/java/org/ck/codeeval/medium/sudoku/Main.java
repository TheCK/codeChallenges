...
@Solution(
    id = 78,
    name = "Sudoku",
    description = "Determine if a grid layout is a valid sudoku solution.",
    url = "https://www.codeeval.com/open_challenges/78/",
          ...
          for (Integer jBox = 0; jBox < dimensionSq; ++jBox) {
            ...
            for (Integer i = 0; i < dimensionSq; ++i) {
              for (Integer j = 0; j < dimensionSq; ++j) {
                if (box.contains(
                    array[(iBox * dimensionSq + i) * dimension + (jBox * dimensionSq + j)])) {
                  valid = false;
                  break;
                }

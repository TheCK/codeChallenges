package org.ck.hackerRank.algorithms.implementation.biggerisgreater;
		...
		try (Scanner in = new Scanner(System.in))
		{
			...
			for (int i = 0; i < testCases; ++i)
			{
				...
				for (int charPos = text.length() - 2; charPos >= 0; --charPos)
				{
					...
					OptionalInt minimalViableChar = text.substring(charPos)
							.chars()
							.filter(charAsInt -> charAsInt > character)
							.min();
					...
					if (minimalViableChar.isPresent())
					{
						...
						greater = Optional.of(text.substring(0, charPos)

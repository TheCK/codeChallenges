  ...
  public static void main(String[] args) {
    try (Scanner in = new Scanner(System.in)) {
      ...
      if (isSorted(array)) {
        ...
        Optional<Pair<Integer, Integer>> swapPoints =
            checkForFixBySwap(Arrays.copyOf(array, length));

        if (swapPoints.isPresent()) {
          System.out.println("yes");
          System.out.println(
              String.format("swap %d %d", swapPoints.get().getKey(), swapPoints.get().getValue()));
        } else {
          Optional<Pair<Integer, Integer>> reversePoints =
              checkForFixByReverse(Arrays.copyOf(array, length));

          if (reversePoints.isPresent()) {
            System.out.println("yes");
            System.out.println(
                String.format(
                    "reverse %d %d", reversePoints.get().getKey(), reversePoints.get().getValue()));
          } else {

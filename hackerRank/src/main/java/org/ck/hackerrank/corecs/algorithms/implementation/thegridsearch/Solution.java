  ...
  public static void main(String[] args) {
    try (Scanner in = new Scanner(System.in)) {
      ...
      for (int testCase = 0; testCase < testCases; ++testCase) {
        ...
        boolean matched =
            isMatched(bigRows, bigColumns, bigArray, smallRows, smallColumns, smallArray);
      ...
      for (int bigColumn = 0; bigColumn < bigColumns - smallColumns + 1; ++bigColumn) {
        ...
        for (int smallRow = 0; smallRow < smallRows; ++smallRow) {
          for (int smallColumn = 0; smallColumn < smallColumns; ++smallColumn) {
            if (bigArray[bigRow + smallRow][bigColumn + smallColumn]
                != smallArray[smallRow][smallColumn]) {
              matched = false;
              break;
            }
          }

    ...
    for (int i = 0; i < GRID.length; ++i) {
      for (int j = 0; j < GRID[i].length; ++j) {
        final long horizontal =
            i < GRID.length - 4 ? GRID[i][j] * GRID[i + 1][j] * GRID[i + 2][j] * GRID[i + 3][j] : 0;
        final long vertical =
            j < GRID[i].length - 4
        ...
        final long diagonal =
            (i < GRID.length - 4 && j < GRID[i].length - 4)
                ? GRID[i][j] * GRID[i + 1][j + 1] * GRID[i + 2][j + 2] * GRID[i + 3][j + 3]
                : 0;
        final long antiDiagonal =
            (i > 3 && j < GRID[i].length - 4)
                ? GRID[i][j] * GRID[i - 1][j + 1] * GRID[i - 2][j + 2] * GRID[i - 3][j + 3]
                : 0;

        maxProduct =
            Math.max(
                Math.max(Math.max(horizontal, vertical), Math.max(diagonal, antiDiagonal)),
                maxProduct);
      }
    }

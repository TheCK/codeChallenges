...
public class HashBreakerReloaded {
  private static final Pattern HASH_PATTERN = Pattern.compile("(?<hash>[a-z0-9]{40})");
  private static final Pattern SALT_PATTERN = Pattern.compile("(?<salt>[a-z0-9]{41,})");
  ...
  public static void main(final String[] args) throws Exception {
    try (final HttpClient client = HttpClient.newHttpClient()) {
      final HttpResponse<String> requestResponse =
          getWebsite("http://challenges.ringzer0ctf.com:10057/", client);

      final Matcher hashMatcher = HASH_PATTERN.matcher(requestResponse.body());
      final Matcher saltMatcher = SALT_PATTERN.matcher(requestResponse.body());

      if (hashMatcher.find() && saltMatcher.find()) {
        final String wantedHash = hashMatcher.group("hash");
        final String salt = saltMatcher.group("salt");

        int i;
        for (i = 0; i < 10_000; ++i) {
          final StringBuilder hash = getHash(i + salt);
        ...
        final HttpResponse<String> responseResponse =
            getWebsite("http://challenges.ringzer0team.com:10057/?r=%s".formatted(i), client);

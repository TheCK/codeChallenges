...
public class HashMeIfYouCanReloaded {
  private static final Pattern MESSAGE_PATTERN = Pattern.compile("(?<message>[01]{20,})");
  ...
  public static void main(final String[] args) throws Exception {
    try (final HttpClient client = HttpClient.newHttpClient()) {
      final HttpResponse<String> requestResponse =
          getWebsite("http://challenges.ringzer0ctf.com:10014/", client);

      final Matcher messageMatcher = MESSAGE_PATTERN.matcher(requestResponse.body());

      if (messageMatcher.find()) {
        final String binaryMessage = messageMatcher.group("message");

        final StringBuilder message = new StringBuilder(binaryMessage.length() / 8);
        for (int i = 0; i < binaryMessage.length(); i += 8) {
          message.append((char) Integer.parseInt(binaryMessage.substring(i, i + 8), 2));
        }

        final StringBuilder hash = getHash(message.toString());

        final HttpResponse<String> responseResponse =
            getWebsite("http://challenges.ringzer0team.com:10014/?r=%s".formatted(hash), client);
